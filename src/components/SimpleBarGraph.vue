  ...
  <div class="simple-bar-graph">
    <svg ref="svg" :width="width + margin.left + margin.right" :height="height + margin.top + margin.bottom">
      <g :style="{ 'transform': `translate(${margin.left}px, ${margin.top}px)` }">
        <rect class="bar" v-for="(item, index) in data" :key="index" :x="x(item.salesperson)" :width="x.bandwidth()" :y="y(item.sales)" :height="getBarHeight(item.sales)" />
...
export default {
  ...
  methods: {
    ...
    getBarHeight(value) {
      const height = this.height - this.y(value);
